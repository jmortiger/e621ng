...
class ActiveSupport::TestCase
  ...
  setup do
    ...
    FileUtils.mkdir_p("#{Rails.root.join('tmp/test-storage2')}")
    storage_manager = StorageManager::Local.new(base_dir: "#{Rails.root.join('tmp/test-storage2')}")
  ...
  teardown do
    ...
    FileUtils.rm_rf("#{Rails.root.join('tmp/test-storage2')}")
    Cache.clear
    RequestStore.clear!
  end
